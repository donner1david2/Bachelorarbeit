...
Die Software des HB10  besteht aus einer Hauptanwendung in welcher der normale Spielbetrieb abläuft, und mehreren \enquote{Satellit}-Anwendungen welche sich auf eine bestimmte Funktionalität spezialisiert sind. Zum Beispiel der Update-Server, welcher dafür zuständig ist, das gesamte System zu updaten.
Für die Videoaufnahme-Funktion des HB10 würde eine solche Anwendung erstellt (siehe Video App in \cref{fig:dartaufbau}).
...
Die Videofunktion des HB10 erstreckt sich über zwei Anwendungen, welche miteinander kommunizieren (s. \cref{fig:videoaufbau}). Die erste ist die eigentliche DART-Anwendung, diese ist dafür zuständig die zweite Anwendung über Ereignisse wie Treffer, Spiel Start und Ende zu Informieren,
...
Die Dart\_App bildet die zentrale Anwendung für den eigentlichen Spielbetrieb des HB10 (s. \cref{fig:dartaufbau}). Sie verwaltet die Spiellogik, verarbeitet spielbezogene Eingaben und stellt die für den Betrieb des Geräts notwendigen Funktionen bereit. 
...
\begin{figure}
    ...
    \caption{Architektur und Datenfluss der Videoaufnahme-Anwendung. Die Anwendung verarbeitet von der Kamera bereitgestellte YUYV-Videodaten, speichert diese zunächst in einem Ringpuffer und erzeugt daraus ereignisbasierte Videoausschnitte sowie fertige Videos. 
...
Im ersten Bereich der Video\_App findet der Aufbau der Verbindung zwischen den beteiligten Komponenten sowie die Herstellung der Betriebsbereitschaft für die spätere Videoaufnahme (s. \cref{fig:dartsequenz1}). Sichtbar wird insbesondere, 
...
Der letzte Teil \cref{fig:dartsequenz3} ist zuständig für den fachlichen Ablauf, der von einem registrierten Wurf bis zur späteren Bereitstellung des zugehörigen Videos reicht. Im Mittelpunkt steht die Zusammenarbeit zwischen \texttt{Dart\_App}, \texttt{Video\_Main}, \texttt{File\_System} und \texttt{Server}. 
...
\begin{figure}[H]
    ...
    \caption{Ablauf der Nachrichtenübertragung zwischen zwei Anwendungen über die Socket-Verbindung. Der Sender serialisiert zunächst die zu übertragenden Daten, übermittelt anschließend die Länge des erzeugten Bytestreams und sendet danach die eigentlichen Nutzdaten. 
...
Für die Kommunikation zwischen Dart\_App und Video\_App wird eine Socket Verbindung verwendet (s. \cref{fig:dartsocket}). Die Video\_App wartet auf eine Verbindung der Dart\_App, und ist Anschließend bereit Ereignisse mit zusätzlichen Daten zu erhalten.
...
Das Benchmark wurde einmal mit dem H.264 hardware Codec der APU des HB10 durchgeführt und ein weiteres mal mit dem x264 Software Codec. Dabei sollten diese ein 78 Sekunden langes Video mit einer Auflösung von 1280x720 bei 10 fps komprimieren. 
Mit hardware Komprimierung wurde eine durchschnittliche CPU Auslastung von 12,13\% mit einer Standardabweichung von 1,25 erreicht, bei einer durchschnittlichen Dauer von 8,59 Sekunden mit einer Standardabweichung von 0,13. 
Im gegensatz wurde mit software Komprimierung eine durchschnittliche CPU Auslastung von 97,10\% Prozent mit einer Standardabweichung von 0,42 erreicht, bei einer durchschnittlichen Dauer von 55,13 Sekunden mit einer Standardabweichung von 0,75.
Die kleinen Schwankungen in der Dauer können auf den scheduler des Betriebssystems zurückgeführt werden.
...
Bei der Hardware Komprimierung hingegen betrug die Maximale CPU Auslastung im Durchschnitt nur 39,70\% mit einer Standardabweichung von 3,35. Wenn man davon ausgeht, 
dass im Normalen Spielbetrieb die CPU Auslastung maximal bei 70\% in einem der vier Kerne liegt, sollte die Hardware Komprimierung problemlos parallel zum normalen Spielbetrieb durchgeführt werden können.
...
ist die Dateigröße kein entscheidender Faktor für die Wahl des Codecs.

Anhand der Ergebnisse des Benchmarks ist zu erkennen, dass die hardware Komprimierung deutlich besser für den Einsatz auf dem HB10 geeignet ist, da die Software Komprimierung die CPU des HB10 komplett auslastet, und somit nicht parallel zum normalen Spielbetrieb durchgeführt werden kann.
Trotzdem konnte im laufe der Arbeit nur die Software Komprimierung auf einer aktuellen Version des HB10 implementiert werden, da die Hardware Komprimierung aufgrund der Treiber-Inkompatibilität auf dem HB10 nicht möglich ist. Deshalb wurde die Software Komprimierung für die Arbeit verwendet, 
und die Hardware Komprimierung als mögliche zukünftige Erweiterung verstanden.

\subsection{Entfernungsbestimmung durch Objekterkennung}
